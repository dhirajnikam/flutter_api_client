...
**Breaking Changes**: None (fully backward compatible with 1.3.0)

This release completes the offline story: local data can now be updated
optimistically, the resulting API call is queued while offline, and the queue
replays automatically when connectivity returns — with fully customizable
...
* **Replay priority** — `QueuedRequest.priority` (default `0`, higher replays
  first; ties break on `createdAt` then `id`). Both queue stores drain in this
...
* **Custom replay ordering** — `OfflineQueueReplayer.compare`, an optional
  `Comparator<QueuedRequest>` that overrides drain order entirely (order by
...
* **Auto-replay on reconnect** — `OfflineAutoReplay`. Feed it a `Stream<bool>`
  ...
  coalescing overlapping triggers so the destructive queue is never drained
  concurrently. `trigger()` is public for a manual "retry now".
